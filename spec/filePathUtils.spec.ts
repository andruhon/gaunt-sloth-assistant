...
describe('filePathUtils', () => {
  ...
  it('gslothDirExists should return true when .gsloth directory exists', async () => {
    fsUtilsMock.existsSync.mockImplementation((path: string) => {
    ...
    expect(fsUtilsMock.existsSync).toHaveBeenCalledWith('/test/project/.gsloth');
  });
  ...
  it('getGslothConfigPath should create .gsloth-settings directory when it does not exist', async () => {
    ...
    let callCount = 0;
    fsUtilsMock.existsSync.mockImplementation((_path: string) => {
      callCount++;
    ...
    expect(fsUtilsMock.mkdirSync).toHaveBeenCalledWith('/test/project/.gsloth/.gsloth-settings', {
      recursive: true,
    });
  });
  ...
  it('getGslothConfigReadPath should return path in .gsloth-settings when config file exists there', async () => {
    ...
    fsUtilsMock.existsSync.mockImplementation((_path: string) => true);
