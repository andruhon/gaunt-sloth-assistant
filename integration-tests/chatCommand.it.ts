...
describe('Chat Command Integration Tests', () => {
  it('should respond to initial message', async () => {
    const output = await runCommandWithArgs('npx', ['gth', 'chat', 'Hello, can you help me?']);
  ...
  it('should create chat history file', async () => {
    const output = await runCommandWithArgs('npx', ['gth', 'chat', 'Test message']);
